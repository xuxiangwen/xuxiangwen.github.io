...
1. $e^{\mathbf i2\pi\mu x}$看成是一个矩阵$\mathbf Q$，行列方向都是无限维的，其列方向是频率$\mu$，行方向是时间$x$。
   ...
   由于$\mathbf Q$是一个[酉矩阵]([https://zh.wikipedia.org/wiki/%E9%85%89%E7%9F%A9%E9%98%B5](https://zh.wikipedia.org/wiki/酉矩阵))，则
   $$
   \mathbf Q^* \mathbf Q = \mathbf {I}
   $$
   

   > $\mathbf {I}$是单位矩阵，$\mathbf Q^* $表示[共轭转置](https://zh.wikipedia.org/wiki/%E5%85%B1%E8%BD%AD%E8%BD%AC%E7%BD%AE)，也就是$e^{-\mathbf i2\pi\mu x}$。
