...
*收益* 是个体寻求最大化未来奖励的函数。 它有几个不同的定义，取决于任务的性质，以及是否希望 *衰减（discount）* 延迟奖励。 无衰减的公式适用于 *情节* 任务（episodic tasks），其中个体和环境的交互可以自然地分解在回合中； 衰减的方案适用于 *持续任务（continuing tasks）*，其互动本身并不会自然地分解在回合中，而是无限制的持续下去。我们定了一组公式，它同时适用于这两种任务。
