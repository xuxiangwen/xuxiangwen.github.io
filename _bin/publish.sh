...
publish_one(){
  ...
  if [[ $file_name =~ [0-9]{4}-[0-9]{2}-[0-9]{2}- ]]; then
    echo can not publish $filepath because it is already a published blog.    
    return
  fi
  ...
  if [[ "${md_date}" = "" ]]; then
    echo can not find correct date as blog publish date in $filepath     
    return    
  fi
...
if [ -d "$file_or_folder" ]; then
  ...
  do
    publish_one $filepath
  done
elif [ -f "$file_or_folder" ]; then
  publish_one $file_or_folder
else
